...
def serve_dataset(filename):
    return send_from_directory(os.path.join(Config.BASE_DIR, 'dataset'), filename)
...
def delete_student(current_user, student_id):
    ...
    if student.face_image_path:
        full_p = os.path.join(Config.BASE_DIR, student.face_image_path)
...
def update_student_face(current_user, student_id):
    ...
    if student.face_image_path:
        old_path = os.path.join(Config.BASE_DIR, student.face_image_path)
...
def process_webcam_frame(current_user, session_id):
    ...
    for st in class_students:
        if st.face_image_path:
            full_p = os.path.join(Config.BASE_DIR, st.face_image_path)
            if os.path.exists(full_p):
                needs_update = True
                if st.encoding_json:
                    try:
