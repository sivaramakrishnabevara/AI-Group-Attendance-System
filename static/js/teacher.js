...
const teacher = {
    ...
    async loadUndetectedGallery() {
        ...
        try {
            const url = this.activeSessionId ? getApiUrl(`/api/undetected/${this.activeSessionId}`) : getApiUrl('/api/undetected');
            const [uRes, stRes] = await Promise.all([
                fetch(url, { headers: { 'Authorization': `Bearer ${auth.token}` } }),
                fetch(getApiUrl('/api/students'), { headers: { 'Authorization': `Bearer ${auth.token}` } })
            ]);
    ...
    async submitManualClaim(undetectedId) {
        ...
        try {
            const res = await fetch(getApiUrl('/api/undetected/claim'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${auth.token}`
